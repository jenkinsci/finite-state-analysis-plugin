...
Per-run flow: resolve project → resolve/create version (`releaseType` from the pre-release flag) → upload + trigger the scan → optionally poll to a terminal state. Upload differs by type: **binary** uses the mb-api upload façade (`POST /scans/upload?projectVersionId=…` with `{filename, types[], multipartUpload, fileSizeBytes}` → direct single-PUT to the returned `uploadUrl`, or S3 multipart via `GET /scans/{scanId}/multipart/{s3UploadId}/{partNumber}/url` per part then `POST /scans/{scanId}/multipart/{s3UploadId}/complete` with an `{eTags: {"1": "…"}}` map → `POST /scans/{scanId}/start`). This single contract is served by both the current backend (as a façade over its native binary service) and the previous backend (natively), so one plugin version works against either. **SBOM and third-party** use the single-shot octet-stream endpoints (`POST /scans/sbom` / `POST /scans/third-party` with the file as the body), where the API uploads to storage server-side and triggers processing in one call — this keeps scanner content off a client→storage PUT that a WAF can block (the failure mode that motivated the choice), at the cost of the API request-body size limit.

Status polling adapts to the backend: the current backend exposes `GET /scans/{id}/status`; the previous backend has no per-scan status endpoint, so the poller falls back to reading `status` from the version's scan list (`GET /scans?filter=projectVersion=="…"`). When `/scans/upload`/single-shot don't return the real scan id (previous backend), it is resolved from that same version-scoped list.
