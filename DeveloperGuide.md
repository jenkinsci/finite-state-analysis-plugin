...
Per-run flow: resolve project → resolve/create version (`releaseType` from the pre-release flag) → create the scan and upload the file (single PUT, or multipart for large binaries) → trigger processing (`/process` or `/process-third-party`) → optionally poll `/scans/{id}/status` to a terminal state.
